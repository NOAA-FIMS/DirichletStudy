# file = build_hake_multinomial_om_4factorial_design_matrix.R
#
# Build a 4-factor, 3-level factorial design matrix for hake.R across:
#   1 = Poisson
#   2 = Lognormal
#   3 = Negative binomial
#
...
#   The four common factorial factors are:
#     factor1 = mean_nsamp
#     factor2 = sigma
#     factor3 = theta_true
#     factor4 = G
#
#   The original factor theta_CV is removed from the experimental design.
#   However, theta_CV is retained as a fixed, non-factor input parameter
#   so hake_multinomial_om.R receives the required key in each .inp file.
#
...
#
# Default row count:
#   - 3 sampling distributions x 3^4 = 243 rows.
#   - 4 sampling distributions x 3^4 = 324 rows when include_loguniform = TRUE.
#
# Main interactive entry point:
#   source("build_hake_multinomial_om_4factorial_design_matrix.R")
...
build_hake_factorial_design_matrix_from_spec <- function(
  ...
  theta_CV_default <- coerce_scalar(theta_CV_default, "theta_CV_default")

  if (lognormal_ln_sd <= 0) stop("lognormal_ln_sd must be positive.")
  if (nb_nb_size <= 0) stop("nb_nb_size must be positive.")
  if (theta_CV_default < 0) stop("theta_CV_default must be nonnegative.")
  ...
  for (blk in needed_blocks) {
    req <- c("mean_nsamp", "sigma", "theta_true")
    if (!identical(sort(names(common_by_block[[blk]])), sort(req))) {
      stop(sprintf("Block '%s' in common_by_block must contain mean_nsamp, sigma, and theta_true only.", blk))
    }
  ...
  build_block <- function(design_block, dist_code, common_vals) {
    ...
    # Row order: factor4 cycles fastest, then factor3, factor2, factor1.
    grid <- expand.grid(
      level4_code = codes,
      level3_code = codes,
      level2_code = codes,
    ...
    grid <- grid[, c("level1_code", "level2_code", "level3_code", "level4_code")]
    ...
    out <- data.frame(
      ...
      G = as.integer(round(G_levels[idx4])),
      h = base_fixed$h,
      theta_true = common_vals$theta_true[idx3],
      theta_CV = theta_CV_default,
      Nmin = 50L,
      Nmax = 100L,
  ...
  forbidden_cols <- c("factor5", "level5_code", "N_range_label")
  if (any(forbidden_cols %in% names(design))) {
    stop("The 4-factor design matrix contains forbidden columns: ",
         paste(intersect(forbidden_cols, names(design)), collapse = ", "))
  }

  expected_rows <- length(needed_blocks) * 3L^4L
  ...
  if (!all(rows_by_block[needed_blocks] == 3L^4L)) {
    stop("Each design block must contain exactly 3^4 = 81 rows.")
  }

  if (!all(design$factor4 == "G")) stop("factor4 must be G for all rows.")
  ...
  legend <- data.frame(
    ...
    Type = c("Fixed baseline and common fourth factor", rep("Factors", length(needed_blocks))),
    Definition = c(
      paste0(
        sprintf(
          "K=%s, h=%s, nsims=%s, random.seed=%s, od_mult=%s, p_lbound=(0.36,0.01,0.0), p_ubound=(0.97,0.59,0.21)",
        ...
        "; fixed theta_CV input parameter: ", fmt_scalar_label(theta_CV_default),
        "; theta_CV removed from factorial design",
...
build_hake_factorial_design_matrix <- function(
  ...
  cat("\nBuild a 4-factor, 3-level factorial design matrix for hake.R\n")
  cat(sprintf(
    "The output will contain %d sampling distributions x 3^4 = %d rows.\n",
    length(selected_blocks), length(selected_blocks) * 3L^4L
  ))
  cat("The common fourth factor is G for all selected sampling distributions.\n")
  cat("The theta_CV factor has been removed from the design but theta_CV is retained as a fixed input parameter.\n")
  ...
  use_equal_common <- prompt_yes_no(
    "Use the same triplets for mean_nsamp, sigma, theta_true, and G across all selected sampling distributions?",
    default = TRUE
  )
  ...
  if (use_equal_common) {
    ...
    G_levels <- prompt_triplet_numeric("G", common_defaults$G, extra_note = "common fourth factor; integer-valued levels")
