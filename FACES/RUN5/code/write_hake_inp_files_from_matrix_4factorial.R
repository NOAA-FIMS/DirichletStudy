...
#     factor4 = G
#
#   The original fourth factor theta_CV is removed from the factorial design.
#   However, theta_CV is retained as a fixed, non-factor input parameter
#   and is written to the .inp files for compatibility with hake_multinomial_om.R.
#   This writer is intended for multinomial_om_flag = 1.
#
...
#  random_seed_generator_seed = 746
# )
#
# Notes:
#   - The required design-matrix columns are checked exactly; theta_CV is
#     retained as a fixed input column, while extra columns such as factor5,
#     level5_code, or N_range_label are not allowed.
...
write_hake_inp_files_from_matrix <- function(matrix_file = "hake_factorial_design_matrix.csv",
  ...
  if (length(multinomial_om_flag) != 1L || is.na(multinomial_om_flag) || multinomial_om_flag != 1L) {
    stop("multinomial_om_flag must be 1 for this multinomial operating-model input writer.")
  }
