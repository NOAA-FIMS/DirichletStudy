# code-B-RE-simplex_emmeans_safe.R
# Memory-safe mixed-effects accuracy analysis using a simplex_id random intercept.
#
# This version is designed for acc_long created by metadata_revised.R as a
# disk-backed Arrow Dataset. It NEVER collects the complete acc_long table.
# Instead, it computes log1p(accuracy) and collapses simulation replicates to
# simplex_id x method x metric cells in Arrow before collecting the reduced table.
...
run_code_B_simplex <- function(
    data_name = "acc_long",
    out_file = "results-code-B-RE-simplex.txt",
    model_file = "fit-code-B-RE-simplex.rds",
    emmeans_metric_file = "emmeans-code-B-RE-simplex-by-metric.csv",
    emmeans_method_file = "emmeans-code-B-RE-simplex-method.csv",
    emmeans_reference_file = "emmeans-code-B-RE-simplex-reference-condition.csv",
    max_simplex = as.integer(Sys.getenv("HAKE_CODE_B_MAX_SIMPLEX", "20000")),
    sample_seed = as.integer(Sys.getenv("HAKE_CODE_B_SAMPLE_SEED", "20260730")),
    save_model = truthy_code_b(Sys.getenv("HAKE_CODE_B_SAVE_MODEL", "true"))) {
...
code_B_simplex_result <- run_code_B_simplex()
