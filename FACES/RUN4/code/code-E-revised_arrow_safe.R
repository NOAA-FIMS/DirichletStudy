# code-E-revised_arrow_safe.R
# Memory-safe GAM analysis for acc_long created by metadata_revised.R.
#
# Compatible with acc_long as either an Arrow Dataset or an in-memory table.
# Arrow input is processed from acc_long RDS partitions one at a time.
#
# Optional controls:
#   Sys.setenv(HAKE_CODE_E_MAX_ROWS = "500000")
#   Sys.setenv(HAKE_CODE_E_MAX_DIFF_ROWS = "300000")
#   Sys.setenv(HAKE_CODE_E_SEED = "20731")
...
run_code_E <- function(data_name = "acc_long",
  ...
  max_rows <- suppressWarnings(as.integer(Sys.getenv("HAKE_CODE_E_MAX_ROWS", "500000")))
  if (is.na(max_rows) || max_rows < 10000L) max_rows <- 500000L
  max_diff_rows <- suppressWarnings(as.integer(Sys.getenv("HAKE_CODE_E_MAX_DIFF_ROWS", "300000")))
  if (is.na(max_diff_rows) || max_diff_rows < 10000L) max_diff_rows <- 300000L
  seed <- suppressWarnings(as.integer(Sys.getenv("HAKE_CODE_E_SEED", "20260731")))
  ...
  tryCatch({
    ...
    saveRDS(list(fit_gam = fit_gam, fit_diff_gam = fit_diff_gam,
            ...
            "results-code-E.rds", compress = "gzip")
