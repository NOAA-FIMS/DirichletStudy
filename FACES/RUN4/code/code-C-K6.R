# code-C1-revised_arrow_safe.R
# Memory-safe pairwise one-sample Hotelling's T^2 / MANOVA tests.
# Compatible with acc_long as an Arrow Dataset or an in-memory table.
...
find_acc_long_parts_C1 <- function() {
  out_dir <- Sys.getenv(
  ...
  if (!length(files)) {
    stop("No acc_long RDS partitions found in: ", part_dir,
         "\nRun metadata_revised.R first, or set HAKE_METADATA_OUTDIR correctly.")
  }
  files
}

standardize_metric_C1 <- function(x) {
...
make_pairwise_diff_partition_C1 <- function(dat, m_target, m_ref, eps = 1e-6) {
  ...
  dt[, metric := standardize_metric_C1(metric)]
...
new_accumulator_C1 <- function() {
  list(n = 0L, sum = numeric(3), crossprod = matrix(0, 3, 3))
}

update_accumulator_C1 <- function(acc, y) {
...
finalize_hotelling_C1 <- function(acc, response_cols = c("rmse", "L1", "Linf")) {
...
process_pair_from_partitions_C1 <- function(files, m_target, m_ref, eps) {
  acc <- new_accumulator_C1()
  for (i in seq_along(files)) {
    message("  ", m_target, "_vs_", m_ref, ": partition ", i, " of ", length(files))
    part <- readRDS(files[[i]])
    diff_part <- make_pairwise_diff_partition_C1(part, m_target, m_ref, eps)
    if (nrow(diff_part)) {
      acc <- update_accumulator_C1(acc,
        diff_part[, c("rmse", "L1", "Linf"), with = FALSE])
    }
    rm(part, diff_part); invisible(gc())
  }
  finalize_hotelling_C1(acc)
}

process_pair_from_memory_C1 <- function(dat, m_target, m_ref, eps) {
  diff_mv <- make_pairwise_diff_partition_C1(dat, m_target, m_ref, eps)
  acc <- update_accumulator_C1(new_accumulator_C1(),
    diff_mv[, c("rmse", "L1", "Linf"), with = FALSE])
  finalize_hotelling_C1(acc)
}

run_code_C1 <- function(data_name = "acc_long", out_file = "results-code-C1.txt", eps = 1e-6) {
  ...
  tryCatch({
    ...
    for (k in seq_along(pairs)) {
      ...
      hotelling <- if (is_arrow_input) {
        process_pair_from_partitions_C1(files, m_target, m_ref, eps)
      } else {
        process_pair_from_memory_C1(acc_long_in, m_target, m_ref, eps)
      }
    ...
    saveRDS(all_results, "results-code-C1.rds", compress = "gzip")
    ...
    fwrite(summary_rows, "results-code-C1-summary.csv")

    cat("Analysis completed successfully.\n")
    cat("Saved detailed results: results-code-C1.rds\n")
    cat("Saved summary table: results-code-C1-summary.csv\n")
