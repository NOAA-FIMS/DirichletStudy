...
run_code_G <- function(
  ...
  tryCatch({
    ...
    max_datasets <- suppressWarnings(as.integer(Sys.getenv("HAKE_CODE_G_MAX_DATASETS", unset="100000")))
    if (is.na(max_datasets) || max_datasets < 1000L) max_datasets <- 100000L
    sample_seed <- suppressWarnings(as.integer(Sys.getenv("HAKE_CODE_G_SEED", unset="20260731")))
    if (is.na(sample_seed)) sample_seed <- 20260731L
    ...
    saveRDS(list(rank_summary=rank_summary, paired_by_metric=paired_by_metric, composite_summary=composite_summary, composite_contrasts=composite_contrasts, fit_comp=fit_comp, emm_comp=emm_comp, metric_model_results=metric_model_results, figure_directory=fig_dir), "results-code-G-revised.rds", compress="gzip")
    cat("\nAnalysis completed successfully.\n")
    cat("Saved model results: results-code-G-revised.rds\n")
