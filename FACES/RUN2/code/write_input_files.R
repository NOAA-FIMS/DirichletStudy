# file = write_input_files.R
# Purpose:
#   1. Build the full design matrix and exclude the Log-uniform block.
#   2. Renumber the remaining 729 experiments and write design_matrix.csv.
#   3. Assign unique random seeds and write faces_ex1.inp through faces_ex729.inp.
...
design_full <- build_design_matrix_default(
  output_csv = "design_matrix_full_4blocks.csv",
  legend_csv = "design_legend_full_4blocks.csv",
...
# Renumber example IDs and input-file names so downstream scripts expect faces_ex1 ... faces_ex729
design_3block$example_id <- seq_len(nrow(design_3block))
design_3block$inp_file <- sprintf("faces_ex%d.inp", design_3block$example_id)
...
# Purpose:
#   Read a faces.R factorial design matrix and write faces_ex1.inp through faces_exN.inp
#   files for use with faces.R and run_examples.R.
#
# Compatibility:
#   This version is compatible with build_design_matrix.R,
#   where G is the common fifth factor for all four sampling distributions:
#     1 = Poisson
#     2 = Lognormal
#     3 = Negative binomial
#     Deprecated = 4 = Log-uniform
#
...
#   write_input_files_from_matrix(
#     matrix_file = "design_matrix.csv",
#     output_dir = ".",
#     validate_common_G = FALSE
#   )
#
# Notes:
#   - Each output file is written in "key = value" format expected by faces.R.
...
write_input_files_from_matrix <- function(matrix_file = "design_matrix.csv",
                                             output_dir = ".",
                                             overwrite = TRUE,
                                             verbose = TRUE,
                                             validate_common_G = FALSE,
  ...
  for (i in seq_len(nrow(design))) {
    ...
    if (!nzchar(out_name)) {
      out_name <- sprintf("faces_ex%d.inp", as.integer(row$example_id[[1]]))
    }
  ...
  summary_file <- file.path(output_dir, "input_file_manifest.csv")
...
# Run the input-file writer after the three-block design matrix has been created.
write_input_files_from_matrix(
  matrix_file = "design_matrix.csv",
  output_dir = ".",
  validate_common_G = FALSE
