...
find_acc_long_parts_G <- function() {
  out_dir <- Sys.getenv(
    "METADATA_OUTDIR",
...
prepare_rank_partition_G <- function(dat, dataset_quota, seed) {
  ...
  if (length(ids)>dataset_quota) { set.seed(seed); ids <- sample(ids, dataset_quota); dt <- dt[dataset_id %chin% ids] }
...
build_rank_sample_G <- function(files, max_datasets, seed) {
  quota <- ceiling(max_datasets/length(files)*1.10)
  pieces <- vector("list", length(files))
  for (i in seq_along(files)) {
    message("Rank data: partition ", i, " of ", length(files))
    part <- readRDS(files[[i]])
    pieces[[i]] <- prepare_rank_partition_G(part, quota, seed+i)
    rm(part); invisible(gc())
  }
  out <- data.table::rbindlist(pieces, use.names=TRUE, fill=TRUE)
  ids <- unique(out$dataset_id)
  if (length(ids)>max_datasets) { set.seed(seed); keep <- sample(ids, max_datasets); out <- out[dataset_id %chin% keep] }
  as.data.frame(out)
}

run_code_F <- function(
  data_name = "acc_long",
  out_file = "results-code-F.txt",
  fig_dir = "figures-code-F"
) {
  ...
  tryCatch({
    cat("Code G: Revised Rank-Based Accuracy Analysis with Three Canonical Metrics\n")
    ...
    max_datasets <- suppressWarnings(as.integer(Sys.getenv("code_F_MAX_DATASETS", unset="100000")))
    if (is.na(max_datasets) || max_datasets < 1000L) max_datasets <- 100000L
    sample_seed <- suppressWarnings(as.integer(Sys.getenv("code_F_SEED", unset="68735")))
    ...
    if (is_arrow_input) {
      ...
      acc_long <- build_rank_sample_G(files, max_datasets, sample_seed)
    } else if (is.data.frame(acc_long_in) || data.table::is.data.table(acc_long_in)) {
      acc_long <- as.data.frame(prepare_rank_partition_G(acc_long_in, max_datasets, sample_seed))
    } else {
    ...
    p_best <- ggplot(rank_summary, aes(x = method, y = prob_best, fill = method)) +
      ...
      guides(fill = "none")
    best_file <- file.path(fig_dir, "code_F_probability_best_by_metric.png")
    ...
    comp_file <- file.path(fig_dir, "code_F_composite_mean_rank.png")
    ...
    dist_file <- file.path(fig_dir, "code_F_rank_distribution_by_metric.png")
    ...
    res_file <- file.path(fig_dir, "code_F_composite_rank_residuals.png")
    save_plot(p_res, res_file)
    cat("Saved plot:", res_file, "\n")

    saveRDS(list(rank_summary=rank_summary, paired_by_metric=paired_by_metric, composite_summary=composite_summary, composite_contrasts=composite_contrasts, fit_comp=fit_comp, emm_comp=emm_comp, metric_model_results=metric_model_results, figure_directory=fig_dir), "code-F-model-results.rds", compress="gzip")
    cat("\nAnalysis completed successfully.\n")
    cat("Saved model results: code-F-model-results.rds\n")
