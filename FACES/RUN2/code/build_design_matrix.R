# file = build_design_matrix.R
#
# Build a 5-factor, 3-level factorial design matrix for faces.R across:
#   1 = Poisson
#   2 = Lognormal
#   3 = Negative binomial
#   4 = Log-uniform
...
#   source("build_design_matrix.R")
#   design <- build_design_matrix_default()
#
# Verification:
#   verify_design_matrix("new_matrix.csv", "reference_matrix.csv")
...
build_design_matrix_from_spec <- function(
    common_by_block,
    G_levels,
    lognormal_ln_sd = 1.0,
    nb_nb_size = 25,
    loguniform_Nmin = 50,
    loguniform_Nmax = 100,
    output_csv = "design_matrix.csv",
    legend_csv = "design_legend.csv",
    verify_against = NULL,
  ...
  design$inp_file <- sprintf("faces_ex%d.inp", design$example_id)
  ...
  legend <- data.frame(
    ...
    Definition = c(
      paste0(
        sprintf(
          "K=%s, h=%s, nsims=%s, random.seed=%s, od_mult=%s, p_lbound=(0.36,0.01,0), p_ubound=(0.97,0.59,0.21)",
  ...
  if (isTRUE(run_verification) && !is.null(verify_against) && file.exists(verify_against)) {
    verification <- verify_design_matrix(output_csv, verify_against)
...
build_design_matrix <- function(
  ...
  cat("\nBuild a 5-factor, 3-level factorial design matrix for faces.R\n")
...
build_design_matrix_default <- function(
    output_csv = "design_matrix.csv",
    legend_csv = "design_legend.csv",
    verify_against = output_csv,
    run_verification = FALSE
...
verify_design_matrix <- function(new_file, reference_file = "design_matrix.csv") {
