...
#      reference grid over G, theta_true, sigma, and mean_nsamp.
...
run_code_G <- function(
  data_name = "acc_long",
  out_file = "results-code-G-run3.txt",
  fig_dir = "figures-code-G-run3"
) {
  ...
  tryCatch({
    ...
    required_cols <- c(
      ...
      "design_block", "G", "theta_true", "sigma",
      "mean_nsamp", "p1", "p2"
    )
    ...
    composite_dat <- rank_dat %>%
      dplyr::group_by(
        dataset_id, example_id, design_block, G, theta_true,
    ...
    fit_comp <- stats::lm(
      mean_rank ~ method * (design_block + G + theta_true + sigma + mean_nsamp + p1 + p2),
      data = composite_dat
    )
    ...
    for (mm in levels(rank_dat$metric)) {
      ...
      cat("\nMetric:", mm, "\n")
      fit_metric <- stats::lm(
        rank ~ method * (design_block + G + theta_true + sigma + mean_nsamp + p1 + p2),
        data = dd
      )
