...
run_code_E <- function(
  data_name = "acc_long",
  out_file = "results-code-E-run3.txt",
  fig_dir = "figures-code-E-run3"
) {
  ...
  tryCatch({
    ...
    fit_gam <- bam(
      y ~ metric * method +
        ...
        design_block + G + theta_true + sigma + mean_nsamp,
      data = acc_gam,
      weights = n,
      method = "fREML",
      discrete = TRUE,
    ...
    diff_v_iii <- acc_long %>%
      ...
      group_by(
        example_id, design_block, G, theta_true, sigma,
