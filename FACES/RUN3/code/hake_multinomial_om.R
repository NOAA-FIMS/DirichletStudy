...
theta_CV_raw <- kv[["theta_CV"]]
theta_CV_missing <- (
  is.null(theta_CV_raw) ||
  length(theta_CV_raw) == 0L ||
  is.na(theta_CV_raw) ||
  !nzchar(trimws(theta_CV_raw))
)
theta_CV <- if (theta_CV_missing) 0 else as.numeric(theta_CV_raw)
...
if (multinomial_om_flag == 0L && theta_CV_missing) {
  stop("theta_CV must be supplied when multinomial_om_flag = 0 because the Dirichlet-multinomial operating model uses theta_CV.")
}
...
if (length(dist_code) != 1L || is.na(dist_code) || !(dist_code %in% seq_along(dist_levels))) {
  stop(sprintf("dist_code must be one of %s; got '%s'.", paste(seq_along(dist_levels), collapse = ", "), paste(dist_code, collapse = ",")))
}
