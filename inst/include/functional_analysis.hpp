...
public:
    ...
    void Finalize()
    {
        ...
        out << "Number of parameters per set: " << parameters.size() << "\n";
        out << "Parameter Range: \n";
        for (int i = 0; i < this->parameters.size(); i++)
        {
            out << std::setw(15) << std::left << this->parameters[i]->GetName() << std::setw(4) << std::left << this->parameters[i]->GetMinBoundary() << "  -   "
                << std::setw(4) << std::left << this->parameters[i]->GetMaxBoundary() << "\n";
        }
        out << "Min function value: " << this->min_value << "\n";
        out << "Max function value: " << this->max_value << "\n";
        out << "Parameter Set for Min function value: \n";
        for (int i = 0; i < this->parameters.size(); i++)
        {
            out << this->parameters[i]->GetName() << "\t" << this->parameter_set_min[i] << std::endl;
        }
        out << "Parameter Set for Max function value: \n";
        for (int i = 0; i < this->parameters.size(); i++)
        {
            out << this->parameters[i]->GetName() << "\t" << this->parameter_set_max[i] << std::endl;
        }
    ...
    const std::vector<std::vector<T>> CovarianceMatrix(const std::vector<T> &x)
    {
        ...
        for (int i = 0; i < this->parameters.size(); i++)
        {
            for (int j = 0; j < this->parameters.size(); j++)
            {
                ...
                if (dxx != dxx)
                { // this is a big hack
                    dxx = std::numeric_limits<T>::lowest();
                }
